...
import MovieCard from './MovieCard.jsx'; // Import your MovieCard component
...
function GenreTitle() {
    ...
    // Pre-filter movies by genre on component mount
    useState(() => {
        const genreMovies = genres.map((genre) =>
            movie.filter((film) => film.genre === genre)
        );
        setFilteredByGenre(genreMovies);
    }, [movie]);
    ...
    return (
        <>
            {filteredByGenre.map((genreMovies, genreIndex) => (
                <div key={genreIndex}>
                    {/* Genre title outside cards-container */}
                    <h1>{genres[genreIndex]}</h1>

                    {/* Horizontal flex group with React Slide */}
